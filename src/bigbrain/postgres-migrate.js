  ...
  try {
    ...
    for (const row of provenance) {
      await postgres.query(`
        ...
        ON CONFLICT (page_slug, event_id) DO UPDATE SET outcome = EXCLUDED.outcome, significance = EXCLUDED.significance
      `, [row.page_slug, row.event_id, row.origin_id, row.listener_id, row.source_type, row.source_label, row.source_message || row.source_label || 'Unknown source message', row.source_icon, row.source_url, row.occurred_at, row.received_at, row.codex_execution_id, row.codex_thread_id, row.raw_ref, row.outcome, row.significance || null, row.commit_message || 'Unknown mutation', row.created_at]);
    }
