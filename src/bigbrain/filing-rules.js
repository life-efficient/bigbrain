...
function defaultTaskSchema() {
  return {
    ...
    guidance: [
      ...
      'Set readiness to underspecified or ready. Readiness is separate from status and records whether the task is specified enough to execute.',
      'Default to readiness: underspecified when context, owner, next action, or completion criteria are missing.',
      'Use readiness: ready only when the task has enough context and completion criteria to be handed off without another clarification round.',
      'Set execution_mode to agent, user, or interactive.',
      'Use execution_mode: agent when an autonomous agent can execute the task.',
      'Use execution_mode: user when the user must personally do the work, such as asking someone for something.',
      'Use execution_mode: interactive when an agent can help only by walking the user through input, review, or decisions.',
