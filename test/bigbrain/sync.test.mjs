...
import { allEmbeddings, getPageRecord, listPageSlugs, openDatabase, replacePageIndex, sqliteRawDatabase } from '../../src/bigbrain/db.js';
...
test('sync returns a report without persisting per-run history', async () => {
...
`);
    ...
    const db = await openDatabase(config);
    const legacyRunsTable = sqliteRawDatabase(db)
      .prepare("SELECT name FROM sqlite_master WHERE type = 'table' AND name = 'sync_runs'")
      .get();
    assert.equal(legacyRunsTable, undefined);
    assert.deepEqual(report.index_totals_after_sync, { pages: 1, links: 0 });
    assert.equal(report.run_work.pages_embedded, 0);
    await db.close?.();
  } finally {
    await fs.rm(fixture.rootDir, { recursive: true, force: true });
  }
