...
- Keep frontmatter, timeline entries, task metadata, raw files, graph data,
  search, dashboard state, and private linked pages private
...
6. If changing visibility, call `set_page_visibility` with:
   - target page slug/path
   - `visibility: "public"` or `visibility: "internal"`
7. Read back or use the tool result to verify the final state.
8. Report:
   - page slug or title
   - previous visibility
   - final visibility
...
Only the page body is public; private linked pages and raw files remain private.
```
